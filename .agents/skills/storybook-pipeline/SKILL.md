...
The slug is derived from the story title or filename: lowercased, hyphenated, ASCII-safe (`^[a-z0-9-]+$`, max 60 chars). Every artifact shares it. All artifacts live in `stories/{slug}/`; the raw `.md` stays at `stories/` (it's the trigger, not an artifact). See [AGENTS.md](../../../AGENTS.md) for the full filename table.
